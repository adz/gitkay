...
/// program. Disposing it cancels every outstanding flow, including ones held by a latest-wins
/// slot. Candidate for extraction into `Axial.Elmish` (depends only on Elmish + Axial).
[<Sealed>]
...
/// Keeps only the most recently started flow alive. Tokens are linked to the owning runtime,
/// so an app-level shutdown cancels whatever the slot currently holds.
[<Sealed>]
