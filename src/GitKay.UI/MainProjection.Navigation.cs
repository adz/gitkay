...
public partial class MainProjection {
    ...
    private List<PaletteItem> BuildCommandItems() {
        ...
        var items = new List<PaletteItem>
        {
            ...
            Command("Open: Git repository…", "Open another local Git repository, with its history", () => WindowCommandRequested?.Invoke("open-repository")),
            Command("Open: folder…", "Browse a folder: its files, previews and syntax highlighting", () => WindowCommandRequested?.Invoke("open-folder")),
            Command("Open: compare two folders…", "Compare two folders, with no repository involved", () => WindowCommandRequested?.Invoke("open-compare")),
            Command("Fetch all remotes", "git fetch --all (respects Git pruning settings)", () => WindowCommandRequested?.Invoke("fetch-all")),
            Command("Fetch and prune stale remote refs", "git fetch --all --prune", () => WindowCommandRequested?.Invoke("fetch-and-prune")),
